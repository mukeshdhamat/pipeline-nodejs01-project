...
class AdminManagementController extends Controller {
  ...
  ********************************************************/
    ...
    /********************************************************
Purpose: Download CSV Of Admin Details Based On Filter In Admin
Method: Post
Authorisation: true
Parameter:
{
...
Return: JSON String
********************************************************/
    async downloadAdminFile() {
        try {
            this.req.body['model'] = Admin;
            this.req.body['fileName'] = 'admins';
            let selectObj = {};
            this.req.body.filteredFields.map(data => {
                selectObj[data] = (data == 'role') ? "$role.role" : 1
            });
            let stages = [{ $match: { isDeleted: false, 'role.role': { $nin: ['Super Admin'] } } }]
            let result = await new CommonService().downloadFiles({ bodyData: this.req.body, selectObj, stages });
